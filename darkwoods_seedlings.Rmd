...
kable(head(darkwoods_seedlings_data))
```
...
```{r, fig.show='hold', out.width='33%', warning=FALSE, message=FALSE}
...
mask_plot1 = readOGR(dsn = "~/Desktop/git_repos/darkwoods_seedlings/plots123.shps/", layer="plot1X")
mask_plot2 = readOGR(dsn = "~/Desktop/git_repos/darkwoods_seedlings/plots123.shps/", layer="plot2X")
mask_plot3 = readOGR(dsn = "~/Desktop/git_repos/darkwoods_seedlings/plots123.shps/", layer="plot3X")
mask_allplots = readOGR(dsn = "~/Desktop/git_repos/darkwoods_seedlings/plots123.shps/", layer="plots123")
#check coordinate reference system
#crs(mask_plot1)
#crs(mask_plot2)
#crs(mask_plot3)
#crs(mask_allplots)
#assign coordinate reference system to 'sp' objects
...
The following code chunk was used to populate descriptive statistics tables 2.1 and 2.2 in Methods section of manuscript draft, which copied directly below for reference. If columns missing, this is a markdown rendering problem, see draft for correct layout
...
| Pinus a.  | 1709 (291.1)  | 10.15 (5.48) | 0.539 (0.060)  | 156.6(100.8) | 5.121 (0.773) | 5.59 (0.34) |
| Tsuga h.  | 1475 (218.0)  | 13.89 (7.79) | 0.504 (0.046)  | 77.4 (28.0)  | 5.001 (0.991) | 4.89 (0.77) |

|           |                           |                         |                            |
|-----------|---------------------------|-------------------------|----------------------------|
|           | **Beetle outbreak stage** | **Fire burn severity**  | **Biogeoclimatic subzone** |
|           | **NA**                    | **Grey**                | **Red**                    |
| Pooled    | 0.72                      | 0.04                    | 0.25                       |
| Plot 1    | 0.55                      | 0.12                    | 0.33                       |
| Plot 2    | 0.49                      | 0.02                    | 0.49                       |
| Plot 3    | 1.00                      | 0.00                    | 0.00                       |
|           |                           |                         |                            |
| Pinus c.  | 0.72                      | 0.04                    | 0.24                       |
| Larix o.  | 0.69                      | 0.05                    | 0.26                       |
| Picea e.  | 0.66                      | 0.04                    | 0.30                       |
| Pseudo m. | 0.74                      | 0.03                    | 0.23                       |
| Pinus p.  | 0.79                      | 0.02                    | 0.19                       |
| Pinus a.  | 0.63                      | 0.04                    | 0.33                       |
| Tsuga h.  | 0.82                      | 0.00                    | 0.18                       |
...
psych::describe(pinus_albi_allplots_sp$distance_m)
...
We test for global and local trends in seedling density using kernel density plots and nearest neighbour models. Non-spatial effects are shown by patterns in absolute density from nearest-neighbour-distance `nndist` metrics between plots and across plots by species and across species pooled.
...
An optimised bandwidth for subsequent spatial modelling is derived using likelihood cross-validation operation `bw.ppl`. Diggle+Berman optimizer written as option but commented out, as no signficant difference found, unless you find otherwise??
...
Kernel density of seedling patterns is modelled using the `epanechnikov` kernel and heat maps. The `epanechnikov` was chosen over `Gaussian` and `Nadaraya-Watson` kernels in favour of its conservative handling of non-negative, outlying estimators and its option of MSE (and RMSE) over MISE metrics. Though, this still needs some discussion. Question raises, should we be testing others and rejigging the results section? I found scare documentation online comparing these kernels.
...
We tested for patterns of dispersion and clustering using monte-carlo simulation of complete spatial randomness model (CSR), repeated 999 times and fitted with non-linear `Kinhom` kernel. Non-linear methods were employed after quadrat test, commented out below, showed intensity was non-constant.

This is done by species and by plots. Though, we have not tested individual species within indivdual plots. As disturbance maps in section 2.4 above show, there are likely advantages for MPB-hosts in plot 3 where there was no MPB-attacks found. If needed, we saved commands below for the Clark+Evan test of aggregation, which is suitable for its use of Donnelly's edge effects constraint (1978) developed to address long boundaries of rectangular field plot sampling.
...
### Seedling Point Processes

Covariates are screened as candidate predictors for final inclusion in the point-pattern-Cox-process models using Likelihood-Ratio-Test that measures their odds of prediction against intercept-only model. Similar to glmnet latticed model, first ones out are dropped from the model.
